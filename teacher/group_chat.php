    ...
    switch ($_REQUEST['action']) { // Use $_REQUEST here too
        case 'get_groups':
            ...
            if ($current_user_id !== null && $link !== false) {
                // Modified query to also get member_count and created_by info
                $sql = "SELECT gc.group_id, gc.group_name, gc.messaging_mode,
                               gc.created_by_ref_id, gc.created_by_type,
                               COUNT(DISTINCT gcm_all.member_ref_id) AS member_count,
                        ...
                        LEFT JOIN group_chat_members gcm_all ON gc.group_id = gcm_all.group_id -- for total member count
                        WHERE gcm_current_user.member_ref_id = ? AND gcm_current_user.member_type = ?
                        GROUP BY gc.group_id, gc.group_name, gc.messaging_mode, gc.created_by_ref_id, gc.created_by_type
                ...
                if ($stmt_check = mysqli_prepare($link, $check_member_sql)) {
                    ...
                    if ($is_member > 0) {
                        $sql = "SELECT gcm.message_id, gcm.sender_type, gcm.sender_ref_id, gcm.message, gcm.sent_at,
                                ...
                                st.current_class AS student_class
        ...
        case 'send_message':
            ...
            if ($groupId !== null && !empty($message) && $current_user_id !== null && $link !== false) {
                // Check for announcements_only mode and user role
                ...
                if ($stmt_mode = mysqli_prepare($link, $mode_sql)) {
                    ...
                    $group_exists = mysqli_stmt_fetch($stmt_mode);
        ...
        case 'create_group':
            ...
            if ($link !== false) {
                ...
                try {
                    // Include created_by_ref_id and created_by_type
                    $sql_group = "INSERT INTO group_chats (group_name, messaging_mode, created_by_role, created_by_ref_id, created_by_type) VALUES (?, ?, ?, ?, ?)";
                    $stmt_group = mysqli_prepare($link, $sql_group);
                    mysqli_stmt_bind_param($stmt_group, "sssis", $groupName, $messagingMode, $current_user_role, $current_user_id, $sender_type);
        ...
        case 'edit_group':
            if (!in_array($current_user_role, ['principal', 'teacher', 'staff'])) {
                echo json_encode(['success' => false, 'error' => 'Permission denied to edit chat groups.']);
                exit;
            }
            $groupId = $_POST['group_id'] ?? null;
            $groupName = trim($_POST['group_name'] ?? '');
            ...
            if (!$groupId || empty($groupName)) {
                echo json_encode(['success' => false, 'error' => 'Invalid group ID or group name.']);
                exit;
            }

            // Verify current user's permission to edit (principal, or the creator)
            $check_permission_sql = "SELECT created_by_ref_id, created_by_type FROM group_chats WHERE group_id = ?";
            $is_allowed_to_edit = false;
            if ($stmt_perm = mysqli_prepare($link, $check_permission_sql)) {
                mysqli_stmt_bind_param($stmt_perm, "i", $groupId);
                mysqli_stmt_execute($stmt_perm);
                mysqli_stmt_bind_result($stmt_perm, $creator_id, $creator_type);
                if (mysqli_stmt_fetch($stmt_perm)) {
                    if ($current_user_role === 'principal' || ($creator_id == $current_user_id && $creator_type == $sender_type)) {
            ...
            if (!$is_allowed_to_edit) {
                echo json_encode(['success' => false, 'error' => 'You do not have permission to edit this group.']);
                exit;
            }
            ...
            if ($link !== false) {
                mysqli_begin_transaction($link);
                try {
                    $sql_update_group = "UPDATE group_chats SET group_name = ?, messaging_mode = ? WHERE group_id = ?";
                    $stmt_update_group = mysqli_prepare($link, $sql_update_group);
                    mysqli_stmt_bind_param($stmt_update_group, "ssi", $groupName, $messagingMode, $groupId);
                    mysqli_stmt_execute($stmt_update_group);
                    mysqli_stmt_close($stmt_update_group);

                    mysqli_commit($link);
                    echo json_encode(['success' => true, 'group_id' => $groupId, 'group_name' => $groupName, 'messaging_mode' => $messagingMode]);
                } catch (Exception $e) {
                    mysqli_rollback($link);
                    error_log("Group update failed: " . $e->getMessage());
                    echo json_encode(['success' => false, 'error' => 'An error occurred during group update. Please try again.']);
                }
            } else {
                echo json_encode(['success' => false, 'error' => 'Database connection error.']);
            }
        ...
        case 'manage_group_members':
            if (!in_array($current_user_role, ['principal', 'teacher', 'staff'])) {
                echo json_encode(['success' => false, 'error' => 'Permission denied to manage group members.']);
                exit;
            }
            $groupId = $_POST['group_id'] ?? null;
            $selectedMembers = json_decode($_POST['members'] ?? '[]', true); // Expected: [['id'=>X, 'type'=>'staff'], ...]

            if (!$groupId || !is_array($selectedMembers)) {
                echo json_encode(['success' => false, 'error' => 'Invalid group ID or members data.']);
                exit;
            }

            // Verify current user's permission to manage members (principal, or the creator)
            $check_permission_sql = "SELECT created_by_ref_id, created_by_type FROM group_chats WHERE group_id = ?";
            $is_allowed_to_edit = false;
            if ($stmt_perm = mysqli_prepare($link, $check_permission_sql)) {
                mysqli_stmt_bind_param($stmt_perm, "i", $groupId);
            ...
            if (!$is_allowed_to_edit) {
                echo json_encode(['success' => false, 'error' => 'You do not have permission to manage members of this group.']);
                exit;
            }
            ...
            if ($link !== false) {
                ...
                try {
                    // Get current members
                    $current_members = [];
                    $sql_get_current = "SELECT member_ref_id, member_type FROM group_chat_members WHERE group_id = ?";
                    if ($stmt_get_current = mysqli_prepare($link, $sql_get_current)) {
                        mysqli_stmt_bind_param($stmt_get_current, "i", $groupId);
                        ...
                        while ($row = mysqli_fetch_assoc($result_current)) {
                            $current_members[implode('_', [$row['member_ref_id'], $row['member_type']])] = $row;
                        }
                        mysqli_stmt_close($stmt_get_current);
                    }

                    // Prepare selected members for easy lookup
                    $new_members_map = [];
                    // Ensure creator and current user (if member) are always included
                    $selectedMembers[] = ['id' => $current_user_id, 'type' => $sender_type]; // Always ensure current user is in selected list
                    $selectedMembers = array_unique($selectedMembers, SORT_REGULAR); // Remove duplicates if any

                    foreach ($selectedMembers as $member) {
                        if (isset($member['id']) && isset($member['type'])) {
                            $new_members_map[implode('_', [$member['id'], $member['type']])] = $member;
                        }
                    }

                    // Determine members to remove
                    $members_to_remove = [];
                    foreach ($current_members as $key => $member) {
                        if (!isset($new_members_map[$key])) {
                            // Don't remove the group creator from the group (if they are removing themselves as principle, they should use delete group)
                            // or the current logged in user (if they are not the creator)
                            // This rule is more for preventing accidental removal of a member from a group when that member is the logged-in user.
                            // If they are the creator, they are explicitly allowed to delete the group, which includes removing themselves.
                            // For simplicity, let's allow removal of anyone for principal, and for creator remove anyone *except* themselves.
                            // Re-evaluate: if a user is trying to edit members and remove themselves, it should succeed.
                            // However, the rule "current user must be a member" is checked on the client side during "get_groups".
                            // If they remove themselves, they won't see the group anymore. This is fine.
                            // The only member who *cannot* be removed is the *creator* if they are managing members and are not a principal.
                            if ($current_user_role !== 'principal' && $member['member_ref_id'] == $creator_id && $member['member_type'] == $creator_type) {
                                // Do not remove the creator if the current user is not a principal.
                                // The principal can remove anyone.
                                continue;
                            }
                    ...
                    $members_to_add = [];
                    foreach ($new_members_map as $key => $member) {
                        if (!isset($current_members[$key])) {
                    ...
                    if (!empty($members_to_remove)) {
                        $sql_delete_members = "DELETE FROM group_chat_members WHERE group_id = ? AND (member_ref_id, member_type) IN (";
                        $placeholders = [];
                        $params = [$groupId];
                        $types = "i";
                        foreach ($members_to_remove as $member) {
                            $placeholders[] = "(?, ?)";
                            $params[] = $member['member_ref_id'];
                            $params[] = $member['member_type'];
                            $types .= "is";
                        }
                        $sql_delete_members .= implode(",", $placeholders) . ")";
                        
                        if ($stmt_delete = mysqli_prepare($link, $sql_delete_members)) {
                            mysqli_stmt_bind_param($stmt_delete, $types, ...$params);
                            mysqli_stmt_execute($stmt_delete);
                            mysqli_stmt_close($stmt_delete);
                        } else {
                            throw new Exception("Failed to prepare delete members query: " . mysqli_error($link));
                        }

                        // Also delete from group_chat_read_status for removed members
                        $sql_delete_read_status = "DELETE FROM group_chat_read_status WHERE group_id = ? AND (member_ref_id, member_type) IN (";
                        $sql_delete_read_status .= implode(",", $placeholders) . ")";
                        if ($stmt_delete_read = mysqli_prepare($link, $sql_delete_read_status)) {
                            mysqli_stmt_bind_param($stmt_delete_read, $types, ...$params);
                            mysqli_stmt_execute($stmt_delete_read);
                            mysqli_stmt_close($stmt_delete_read);
                        } else {
                            throw new Exception("Failed to prepare delete read status query: " . mysqli_error($link));
                        }
                    }

                    // Execute additions
                    if (!empty($members_to_add)) {
                        $sql_insert_member = "INSERT INTO group_chat_members (group_id, member_type, member_ref_id) VALUES (?, ?, ?)";
                        $stmt_insert_member = mysqli_prepare($link, $sql_insert_member);
                        $sql_insert_read_status = "INSERT INTO group_chat_read_status (group_id, member_ref_id, member_type, last_read_message_id) VALUES (?, ?, ?, 0)";
                        $stmt_insert_read_status = mysqli_prepare($link, $sql_insert_read_status);

                        foreach ($members_to_add as $member) {
                            mysqli_stmt_bind_param($stmt_insert_member, "isi", $groupId, $member['type'], $member['id']);
                            mysqli_stmt_execute($stmt_insert_member);
                            mysqli_stmt_bind_param($stmt_insert_read_status, "iis", $groupId, $member['id'], $member['type']);
                            mysqli_stmt_execute($stmt_insert_read_status);
                        }
                        mysqli_stmt_close($stmt_insert_member);
                        mysqli_stmt_close($stmt_insert_read_status);
                    }

                    mysqli_commit($link);
                    echo json_encode(['success' => true, 'added' => count($members_to_add), 'removed' => count($members_to_remove)]);

                } catch (Exception $e) {
                    mysqli_rollback($link);
                    error_log("Group member management failed: " . $e->getMessage());
                    echo json_encode(['success' => false, 'error' => 'An error occurred during member management. Please try again.']);
                }
            } else {
                echo json_encode(['success' => false, 'error' => 'Database connection error.']);
            }
        ...
        case 'delete_group':
            if (!in_array($current_user_role, ['principal', 'teacher', 'staff'])) {
                echo json_encode(['success' => false, 'error' => 'Permission denied to delete chat groups.']);
                exit;
            }
            $groupId = $_POST['group_id'] ?? null;
            if (!$groupId) {
            ...
            // Verify current user's permission to delete (principal, or the creator)
            $check_permission_sql = "SELECT created_by_ref_id, created_by_type FROM group_chats WHERE group_id = ?";
            ...
            if ($link !== false) {
                mysqli_begin_transaction($link);
                try {
                    // Manually delete from dependent tables if ON DELETE CASCADE is not set
                    $delete_deps = [
                        "DELETE FROM group_chat_messages WHERE group_id = ?",
                        "DELETE FROM group_chat_members WHERE group_id = ?",
                        "DELETE FROM group_chat_read_status WHERE group_id = ?"
                    ];
                    foreach ($delete_deps as $sql_dep) {
                        if ($stmt_dep = mysqli_prepare($link, $sql_dep)) {
                            mysqli_stmt_bind_param($stmt_dep, "i", $groupId);
                            mysqli_stmt_execute($stmt_dep);
                            mysqli_stmt_close($stmt_dep);
                        } else {
                            throw new Exception("Failed to prepare dependent delete query: " . mysqli_error($link));
                        }
                    }

                    $sql_delete_group = "DELETE FROM group_chats WHERE group_id = ?";
                    if ($stmt_group = mysqli_prepare($link, $sql_delete_group)) {
                        mysqli_stmt_bind_param($stmt_group, "i", $groupId);
                        mysqli_stmt_execute($stmt_group);
                        mysqli_stmt_close($stmt_group);
                    } else {
                        throw new Exception("Failed to prepare group delete query: " . mysqli_error($link));
                    }
                    ...
                    echo json_encode(['success' => false, 'error' => 'An error occurred during group deletion. Please try again.']);
                }
            } else {
                echo json_encode(['success' => false, 'error' => 'Database connection error.']);
            }
        ...
        case 'get_users_for_group_creation': // Used for both create and edit modals
            if (!in_array($current_user_role, ['principal', 'teacher', 'staff'])) {
            ...
            if ($link !== false) {
                ...
                if ($stmt_staff = mysqli_prepare($link, $sql_staff)) {
                    ...
                    while ($row = mysqli_fetch_assoc($result_staff)) {
                        $response_data['staff'][] = ['id' => $row['id'], 'name' => $row['name'] . ' (Staff - ' . ucfirst($row['role']) . ')', 'type' => 'staff', 'role' => $row['role']];
                    }
                ...
                if ($stmt_students = mysqli_prepare($link, $sql_students)) {
                    ...
                    while ($row = mysqli_fetch_assoc($result_students)) {
                        $response_data['students'][] = ['id' => $row['id'], 'name' => $row['name'] . ' (Student - Class ' . $row['current_class'] . ')', 'type' => 'student', 'class' => $row['current_class']];
                    }
        ...
        case 'get_group_members':
            ...
            $members = [];
            if ($groupId !== null && $current_user_id !== null && $link !== false) {
                 // Security check: Verify user is a member of the group they are trying to view members for
                $check_member_sql = "SELECT COUNT(*) FROM group_chat_members WHERE group_id = ? AND member_ref_id = ? AND member_type = ?";
                if ($stmt_check = mysqli_prepare($link, $check_member_sql)) {
                    mysqli_stmt_bind_param($stmt_check, "iis", $groupId, $current_user_id, $sender_type);
                    mysqli_stmt_execute($stmt_check);
                    ...
                    if ($is_member > 0) {
                        $sql = "SELECT gcm.member_type, gcm.member_ref_id,
                                COALESCE(s.staff_name, st.full_name) AS member_name,
                                COALESCE(s.role, 'student') AS member_role,
                                st.current_class AS member_class
                                FROM group_chat_members gcm
                                LEFT JOIN staff s ON gcm.member_type = 'staff' AND gcm.member_ref_id = s.staff_id
                                LEFT JOIN students st ON gcm.member_type = 'student' AND gcm.member_ref_id = st.user_id
                                WHERE gcm.group_id = ?
                        ...
                        if ($stmt = mysqli_prepare($link, $sql)) {
                            ...
                            if (mysqli_stmt_execute($stmt)) {
                                ...
                                error_log("Failed to execute get_group_members query: " . mysqli_error($link));
                                echo json_encode(['success' => false, 'error' => 'Failed to retrieve members.']);
                            }
                            mysqli_stmt_close($stmt);
                        } else {
                            error_log("Failed to prepare get_group_members query: " . mysqli_error($link));
                            echo json_encode(['success' => false, 'error' => 'Internal server error.']);
                        }
                    } else {
                        echo json_encode(['success' => false, 'error' => 'You are not a member of this group.']);
                     ...
                     error_log("Failed to prepare member check query for get_group_members: " . mysqli_error($link));
                     echo json_encode(['success' => false, 'error' => 'Internal server error.']);
                }
            } else {
                echo json_encode(['success' => false, 'error' => 'Invalid group ID.']);
...
<head>
    ...
    <style>
        ...
        .group-item.active .group-member-count { background-color: #a7d9ed; color: #0e7490; }
...
<body>
    ...
    $navbar_path = "./staff_navbar.php"; // Assuming this is the correct path for all roles
            ...
            <div id="chatWindowSection" class="chat-window hidden-on-mobile">
                <div id="chatHeader" class="chat-header">
                    ...
                    <div class="flex-grow">
                        ...
                        <span id="chatGroupMembersCount" class="text-sm font-normal text-gray-500 ml-2"></span>
                    </div>
                    <div class="chat-header-actions" id="chatHeaderActions">
                         <?php if (in_array($current_user_role, ['principal', 'teacher', 'staff'])): ?>
                            <button id="editGroupBtn" style="display: none;"><i class="fas fa-edit"></i> Edit</button>
                         <?php endif; ?>
                        <button id="viewMembersBtn" style="display: none;"><i class="fas fa-users"></i> Info</button>
                    </div>
                </div>
    ...
    <div id="createGroupModal" class="modal">
        <div class="modal-content">
            <span class="close-button" id="closeCreateModalBtn">&times;</span>
    ...
    <div id="editGroupModal" class="modal">
        <div class="modal-content">
            ...
            <h2 class="text-2xl font-bold mb-5 text-gray-800">Edit Group Settings</h2>
            <form id="editGroupForm">
                <input type="hidden" id="editGroupId">
                <div class="form-control">
                    <label for="editGroupName">Group Name</label>
                    <input type="text" id="editGroupName" name="group_name" required placeholder="e.g., Grade 10 English Class">
                </div>
                <div class="form-control">
                    <label class="flex items-center cursor-pointer text-gray-700">
                        <input type="checkbox" id="editAnnouncementsOnlyMode" name="messaging_mode" value="announcements_only" class="mr-2 h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
                        <span>Announcements-Only Mode (only staff can send messages)</span>
                    </label>
                </div>
                <div class="form-control">
                    <label>Filter Members By:</label>
                    <div id="editMemberFilterContainer" class="flex flex-wrap gap-4 mt-2">
                        <label class="inline-flex items-center"><input type="radio" name="edit_member_filter" value="all" checked class="mr-2 h-4 w-4 text-indigo-600"> All</label>
                        <label class="inline-flex items-center"><input type="radio" name="edit_member_filter" value="staff" class="mr-2 h-4 w-4 text-indigo-600"> Staff Only</label>
                        <label class="inline-flex items-center"><input type="radio" name="edit_member_filter" value="student" class="mr-2 h-4 w-4 text-indigo-600"> Students Only</label>
                    </div>
                </div>
                <div class="form-control">
                    <label for="editClassFilterSelect">Quick-Select Students by Class</label>
                    <select id="editClassFilterSelect" class="block w-full py-2 px-3 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        <option value="">-- Select a Class to Filter --</option>
                    </select>
                </div>
                <div class="form-control">
                    <label for="editGroupMembers">Manage Members</label>
                    <select id="editGroupMembers" name="members[]" multiple required class="form-control">
                        <option value="">Loading users...</option>
                    </select>
                    <p class="text-xs text-gray-500 mt-1">Hold Ctrl/Cmd (Windows/Mac) to select/deselect multiple members.</p>
                </div>
                <button type="submit" class="btn-modal-submit mt-4"><i class="fas fa-save mr-2"></i> Save Changes</button>
                <button type="button" id="deleteGroupBtn" class="btn-delete-group"><i class="fas fa-trash-alt mr-2"></i> Delete Group</button>
            </form>
        </div>
    </div>
    ...
    <div id="viewMembersModal" class="modal">
        <div class="modal-content max-w-sm">
            <span class="close-button" id="closeViewMembersModalBtn">&times;</span>
            <h2 class="text-2xl font-bold mb-5 text-gray-800">Group Members</h2>
            <div id="membersList" class="max-h-80 overflow-y-auto">
                <p class="loading-text"><i class="fas fa-spinner fa-spin mr-2"></i> Loading members...</p>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const chatGroupMembersCountSpan = document.getElementById('chatGroupMembersCount');
        const chatHeaderActionsDiv = document.getElementById('chatHeaderActions');
        ...
        const editAnnouncementsOnlyModeCheckbox = document.getElementById('editAnnouncementsOnlyMode');
        const editGroupMembersSelect = document.getElementById('editGroupMembers');
        const editClassFilterSelect = document.getElementById('editClassFilterSelect');
        const editMemberFilterContainer = document.getElementById('editMemberFilterContainer');
        const deleteGroupBtn = document.getElementById('deleteGroupBtn');

        // View Members Modal elements
        const viewMembersModal = document.getElementById('viewMembersModal');
        const closeViewMembersModalBtn = document.getElementById('closeViewMembersModalBtn');
        ...
        let lastMessageId = 0;
        let groupDetails = {}; // Stores group info: {name, mode, member_count, created_by_ref_id, created_by_type}
        ...
        let allUsersForCreation = { staff: [], students: [], classes: [] }; // Global store for user list in modals
                    ...
                    if (data.groups.length > 0) {
                        data.groups.forEach(group => {
                            groupDetails[group.group_id] = { 
                                name: escapeHtml(group.group_name), 
                                mode: group.messaging_mode, 
                                unread_count: group.unread_count,
                                member_count: group.member_count,
                                created_by_ref_id: group.created_by_ref_id,
                            ...
                            let groupNameHtml = `<div class="flex-grow truncate pr-2">
                                                    <span class="text-gray-800">${escapeHtml(group.group_name)}</span>
                                                </div>`;
                            
                            let indicatorsHtml = '<div class="flex items-center gap-2">';
                            if (group.messaging_mode === 'announcements_only') {
                                indicatorsHtml += `<span class="group-mode-indicator flex-shrink-0" title="Announcements Only"><i class="fas fa-bullhorn"></i></span>`;
                            }
                            indicatorsHtml += `<span class="group-member-count flex-shrink-0" title="${group.member_count} members"><i class="fas fa-users"></i> ${group.member_count}</span>`;
                            ...
                            groupItem.innerHTML = `${groupNameHtml}${indicatorsHtml}`;
                        ...
                        if (selectedGroupId === null || !groupDetails[selectedGroupId]) {
                           selectGroup(data.groups[0].group_id, true);
                            ...
                            if (activeItem) {
                                ...
                                // If previously selected group was deleted
                                selectGroup(data.groups[0].group_id, true);
                            }
                        }
                    } else {
                        ...
                        chatGroupMembersCountSpan.textContent = '';
        ...
        async function selectGroup(groupId, forceReload = false) {
            ...
            chatGroupMembersCountSpan.textContent = `(${groupInfo.member_count} members)`;
            messagesContainer.innerHTML = '<p class="loading-text"><i class="fas fa-spinner fa-spin mr-2"></i> Loading messages...</p>';

            // Show/hide edit and view members buttons
            const isStaffOrPrincipal = ['principal', 'teacher', 'staff'].includes(currentUserRole);
            const isGroupCreator = (groupInfo.created_by_ref_id == currentUserId && groupInfo.created_by_type == currentSenderType);
            ...
            viewMembersBtn.style.display = 'inline-flex';
        ...
        function displayMessageInChat(msg) {
            ...
            let senderDetails = '';
            if (msg.sender_type === 'staff' && msg.sender_role) {
                senderDetails = ` (${ucfirst(escapeHtml(msg.sender_role))})`;
            } else if (msg.sender_type === 'student' && msg.student_class) {
                senderDetails = ` (Class ${escapeHtml(msg.student_class)})`;
            }
            ...
            if (!isSent) {
                ...
                senderDiv.textContent = `${senderName}${senderDetails}`;
        ...
        if (createGroupBtn) { 
            createGroupBtn.addEventListener('click', async () => { 
                createGroupForm.reset(); // Clear previous data
                announcementsOnlyModeCheckbox.checked = false;
                document.querySelector('#memberFilterContainer input[value="all"]').checked = true; // Default filter
                openModal(createGroupModal); 
                await loadUsersForGroupCreation(newGroupMembersSelect, classFilterSelect, memberFilterContainer); 
            }); 
        }
        closeCreateModalBtn.addEventListener('click', () => closeModal(createGroupModal));
        ...
        if (editGroupBtn) {
            editGroupBtn.addEventListener('click', async () => {
                const groupInfo = groupDetails[selectedGroupId];
                if (!groupInfo) {
                    displayMessage('No group selected or group details unavailable.', 'error');
                    return;
                }
                
                editGroupIdInput.value = selectedGroupId;
                editGroupNameInput.value = groupInfo.name;
                editAnnouncementsOnlyModeCheckbox.checked = (groupInfo.mode === 'announcements_only');

                // Determine if the current user can delete the group (principal or creator)
                const isPrincipal = currentUserRole === 'principal';
                const isCreator = (groupInfo.created_by_ref_id == currentUserId && groupInfo.created_by_type == currentSenderType);
                ...
                // Load and pre-select members
                openModal(editGroupModal);
                await loadUsersForGroupCreation(editGroupMembersSelect, editClassFilterSelect, editMemberFilterContainer);
                await preSelectGroupMembers(selectedGroupId, editGroupMembersSelect);
            });
        }
        closeEditModalBtn.addEventListener('click', () => closeModal(editGroupModal));
        ...
        if (viewMembersBtn) {
            viewMembersBtn.addEventListener('click', async () => {
                if (!selectedGroupId) {
                    displayMessage('No group selected.', 'error');
                    return;
                }
        ...
        closeViewMembersModalBtn.addEventListener('click', () => closeModal(viewMembersModal));
        viewMembersModal.addEventListener('click', (event) => {
        ...
        // Load users for group creation/edit modals
        async function loadUsersForGroupCreation(memberSelectElement, classSelectElement, filterContainerElement) {
             memberSelectElement.innerHTML = '<option value="">Loading users...</option>'; 
             classSelectElement.innerHTML = '<option value="">-- Select a Class to Filter --</option>';
             try {
                const response = await fetch('group_chat.php?action=get_users_for_group_creation');
                const result = await response.json();
                memberSelectElement.innerHTML = '';
                if (result.success) {
                    allUsersForCreation.staff = result.data.staff; 
                    allUsersForCreation.students = result.data.students;
                    allUsersForCreation.classes = result.data.classes;
                    ...
                    // Reset filter to 'all' and display initially
                    const allRadio = filterContainerElement.querySelector('input[value="all"]');
                    if(allRadio) allRadio.checked = true;
                    filterAndDisplayMembers(memberSelectElement, 'all'); 
                } else { 
                    memberSelectElement.innerHTML = `<option value="">Error: ${escapeHtml(result.error)}</option>`; 
                }
            } catch (error) { 
                memberSelectElement.innerHTML = `<option value="">Network error.</option>`; 
                console.error('Error loading users for group creation:', error);
            }
        }

        // Filter members for group creation/edit
        function filterAndDisplayMembers(memberSelectElement, filterType) {
            memberSelectElement.innerHTML = ''; 
            let usersToDisplay = [];
            if (filterType === 'all') { 
            ...
            usersToDisplay.forEach(user => {
                if (user.id == currentUserId && user.type == currentSenderType) { 
                    return; // Skip current user from selectable list to avoid confusion, they are implicitly added/managed server-side
                }
                const option = document.createElement('option'); 
            ...
            if (usersToDisplay.length === 0) { 
                memberSelectElement.innerHTML = '<option value="">No users found for this filter.</option>'; 
            }
        }

        // Pre-select existing members in edit modal
        async function preSelectGroupMembers(groupId, memberSelectElement) {
            try {
                const response = await fetch(`group_chat.php?action=get_group_members&group_id=${groupId}`);
                const data = await response.json();
                if (data.success) {
                    const currentMembersMap = new Map();
                    data.members.forEach(member => {
                        currentMembersMap.set(`${member.member_ref_id}_${member.member_type}`, true);
                    });

                    Array.from(memberSelectElement.options).forEach(option => {
                        try {
                            const memberData = JSON.parse(option.value);
                            // Pre-select if it's an existing member AND not the current user (current user is special handled)
                            if (currentMembersMap.has(`${memberData.id}_${memberData.type}`)) {
                    ...
                    displayMessage('Failed to load current group members for editing: ' + (data.error || 'Unknown error.'), 'error');
                }
            } catch (error) {
                console.error('Network error fetching current members:', error);
                displayMessage('Network error loading current members for editing.', 'error');
            }
        }
        ...
        editGroupForm.addEventListener('submit', async (e) => {
            ...
            const messagingMode = editAnnouncementsOnlyModeCheckbox.checked ? 'announcements_only' : 'open_chat';
            const selectedOptions = Array.from(editGroupMembersSelect.selectedOptions);
            const memberIds = selectedOptions.map(option => JSON.parse(option.value));
            ...
            const submitBtn = editGroupForm.querySelector('button[type="submit"]');
            submitBtn.disabled = true;
            submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Saving...';

            try {
                // First update group name/mode
                const formDataGroup = new FormData();
                formDataGroup.append('action', 'edit_group');
                formDataGroup.append('group_id', groupId);
                formDataGroup.append('group_name', groupName);
                formDataGroup.append('messaging_mode', messagingMode);

                const responseGroup = await fetch('group_chat.php', { method: 'POST', body: formDataGroup });
                const dataGroup = await responseGroup.json();

                if (!dataGroup.success) {
                    displayMessage('Error updating group details: ' + (dataGroup.error || 'Unknown error.'), 'error');
                    return;
                }

                // Then update members
                const formDataMembers = new FormData();
                formDataMembers.append('action', 'manage_group_members');
                formDataMembers.append('group_id', groupId);
                formDataMembers.append('members', JSON.stringify(memberIds));

                const responseMembers = await fetch('group_chat.php', { method: 'POST', body: formDataMembers });
                const dataMembers = await responseMembers.json();

                if (dataMembers.success) {
                    displayMessage(`Group "${escapeHtml(groupName)}" updated successfully!`, 'success');
                    closeModal(editGroupModal);
                    await loadGroups(); // Reload groups to show updated names/counts
                    selectGroup(groupId, true); // Re-select to refresh chat header
                } else {
                    displayMessage('Error managing group members: ' + (dataMembers.error || 'Unknown error.'), 'error');
                }
            } catch (error) {
                displayMessage('Network error updating group.', 'error');
        ...
        deleteGroupBtn.addEventListener('click', async () => {
            ...
            if (!confirm(`Are you sure you want to delete the group "${escapeHtml(groupName)}"? This action cannot be undone.`)) {
                return;
            }

            deleteGroupBtn.disabled = true;
            deleteGroupBtn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> Deleting...';
            ...
            try {
                ...
                if (data.success) {
                    displayMessage(`Group "${escapeHtml(groupName)}" deleted successfully!`, 'success');
                    closeModal(editGroupModal);
                    selectedGroupId = null; // Clear selected group
                    if (pollingInterval) clearInterval(pollingInterval);
                    await loadGroups(); // Reload groups to reflect deletion
                } else {
                    displayMessage('Error deleting group: ' + (data.error || 'Unknown error.'), 'error');
                }
            } catch (error) {
                displayMessage('Network error deleting group.', 'error');
                console.error('Error deleting group:', error);
            } finally {
                deleteGroupBtn.disabled = false;
                deleteGroupBtn.innerHTML = '<i class="fas fa-trash-alt mr-2"></i> Delete Group';
            }
        });

        // Quick-select students by class (for Create Group)
        classFilterSelect.addEventListener('change', (e) => {
            const selectedClass = e.target.value;
            quickSelectMembersByClass(newGroupMembersSelect, selectedClass);
        });

        // Quick-select students by class (for Edit Group)
        editClassFilterSelect.addEventListener('change', (e) => {
            const selectedClass = e.target.value;
            quickSelectMembersByClass(editGroupMembersSelect, selectedClass);
        });

        function quickSelectMembersByClass(memberSelectElement, selectedClass) {
            ...
            if (selectedClass) { 
                Array.from(memberSelectElement.options).forEach(option => { 
                    const userData = allUsersForCreation.students.find(s => option.textContent.includes(`(Student - Class ${selectedClass})`) && option.textContent.includes(s.name));
                    if (userData) { 
                        option.selected = true; 
                    } 
                }); 
        ...
        // Member filter (All/Staff/Student) radio buttons (for Create Group)
        memberFilterContainer.addEventListener('change', e => { 
            const filter = e.target.value; 
            if (!filter) return; 
            filterAndDisplayMembers(newGroupMembersSelect, filter); 
            classFilterSelect.value = ""; // Clear class filter when main filter changes
        });

        // Member filter (All/Staff/Student) radio buttons (for Edit Group)
        editMemberFilterContainer.addEventListener('change', async e => { 
            const filter = e.target.value; 
            if (!filter) return; 
            filterAndDisplayMembers(editGroupMembersSelect, filter); 
            editClassFilterSelect.value = ""; // Clear class filter when main filter changes
            // After filtering, re-apply pre-selection based on current group members
            await preSelectGroupMembers(selectedGroupId, editGroupMembersSelect);
        });

        // --- Load Group Members for View Modal ---
        async function loadGroupMembers(groupId) {
            membersListDiv.innerHTML = '<p class="loading-text"><i class="fas fa-spinner fa-spin mr-2"></i> Loading members...</p>';
            try {
                const response = await fetch(`group_chat.php?action=get_group_members&group_id=${groupId}`);
                const data = await response.json();
                if (data.success) {
                    ...
                    if (data.members.length > 0) {
                        data.members.forEach(member => {
                            ...
                            memberDiv.className = 'flex items-center p-2 border-b border-gray-100 last:border-b-0';
                            let icon = '<i class="fas fa-user-circle mr-2 text-gray-500"></i>';
                            let details = '';
                            if (member.member_type === 'staff') {
                                icon = '<i class="fas fa-chalkboard-teacher mr-2 text-blue-500"></i>';
                                details = `(Staff - ${ucfirst(escapeHtml(member.member_role))})`;
                            } else if (member.member_type === 'student') {
                                icon = '<i class="fas fa-user-graduate mr-2 text-green-500"></i>';
                                details = `(Student - Class ${escapeHtml(member.member_class)})`;
                            }
                            memberDiv.innerHTML = `${icon} <span class="font-medium text-gray-700">${escapeHtml(member.member_name)}</span> <span class="text-sm text-gray-500 ml-auto">${details}</span>`;
                            membersListDiv.appendChild(memberDiv);
                        });
                    } else {
                        membersListDiv.innerHTML = `<p class="empty-state"><i class="fas fa-users-slash"></i> No members found for this group.</p>`;
                    }
                } else {
                    membersListDiv.innerHTML = `<p class="empty-state">Error: ${escapeHtml(data.error || 'Failed to load members.')}</p>`;
                    displayMessage('Failed to load group members: ' + (data.error || 'Unknown error.'), 'error');
                }
            } catch (error) {
                membersListDiv.innerHTML = `<p class="empty-state">Network error.</p>`;
                console.error('Error fetching group members:', error);
                displayMessage('Network error fetching group members.', 'error');
            }
        }
