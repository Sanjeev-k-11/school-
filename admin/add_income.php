...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true || ($_SESSION['role'] !== 'admin' && $_SESSION['role'] !== 'principal')) {
    $_SESSION['operation_message'] = "<p class='text-red-600'>Access denied. Only Admins and Principals can add income.</p>";
...
$income_date = $description = $category = $amount = $payment_method = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($income_date_err) && empty($description_err) && empty($amount_err) && empty($payment_method_err)) {

         // Check if database connection is available
        if ($link === false) {
             $operation_message = "<p class='text-red-600'>Database connection error. Could not add income.</p>";
             error_log("Add Income DB connection failed: " . mysqli_connect_error());
        } else {

             // Prepare an insert statement
             $sql = "INSERT INTO income (income_date, description, category, amount, payment_method, recorded_by_user_id) VALUES (?, ?, ?, ?, ?, ?)";
             ...
             if ($stmt = mysqli_prepare($link, $sql)) {
                 ...
                 if (mysqli_stmt_execute($stmt)) {
                     // Record added successfully. Redirect to manage income page.
                     $_SESSION['operation_message'] = "<p class='text-green-600'>Income added successfully.</p>";
                     header("location: manage_income.php");
                     exit;
                 } else {
                     $operation_message = "<p class='text-red-600'>Error adding income: " . htmlspecialchars(mysqli_stmt_error($stmt)) . "</p>";
                      error_log("Error executing add income query: " . mysqli_stmt_error($stmt));
                 }
                 ...
                 $operation_message = "<p class='text-red-600'>Database statement preparation failed: " . htmlspecialchars(mysqli_error($link)) . "</p>";
                  error_log("Error preparing add income statement: " . mysqli_error($link));
            }
         ...
         <div class="bg-white p-6 rounded-xl shadow-md max-w-md mx-auto">
              ...
              <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" novalidate>
                 ...
                 <div class="form-group text-center mt-6">
                     <button type="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-md focus:outline-none focus:shadow-outline transition duration-200">
                         Add Income
                     </button>
                 </div>
