...
$email = ''; // NEW: For displaying the submitted email
...
$email_for_db = null; // NEW: For storing email in DB (can be null if optional)
...
$virtual_id_err = "";
$password_err = "";
$email_err = ""; // NEW: Error variable for email
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // --- NEW: Email Address (Optional, but must be unique if provided) ---
    $email = trim($_POST["email"] ?? ''); // Assign to $email for display/retention
    if (!empty($email)) {
            ...
            if ($link !== false) { // Ensure DB link is valid
                ...
                if ($check_stmt = mysqli_prepare($link, $check_sql)) {
                    ...
                    if (mysqli_stmt_execute($check_stmt)) {
                        ...
                        $email_err = "Database error during email check: " . mysqli_stmt_error($check_stmt);
                        error_log("Database error during email check: " . mysqli_stmt_error($check_stmt));
                    }
                    mysqli_stmt_close($check_stmt);
                } else {
                    $email_err = "Database error preparing email check: " . mysqli_error($link);
                    error_log("Database error preparing email check: " . mysqli_error($link));
                }
            } else {
                $email_err = "Database connection error prevented email uniqueness check.";
            }
        }
        $email_for_db = $email; // Use the sanitized and validated email for DB
    } else {
        $email_for_db = null; // Ensure it's null for the DB if empty
    }
    ...
    $has_errors = !empty($full_name_err) || !empty($father_name_err) || !empty($mother_name_err) || !empty($phone_number_err) ||
                  ...
                  !empty($email_err); // NEW: Check for email error


    if (!$has_errors) {

        // --- 7. Database Insertion ---
        // Update the SQL query to include the 'email' column
        $sql = "INSERT INTO students (full_name, father_name, mother_name, email, phone_number, virtual_id, password, whatsapp_number, current_class, previous_class, previous_school, previous_marks_percentage, current_marks, address, pincode, state, photo_filename, roll_number, village, date_of_birth, takes_van) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"; // Added email
        ...
        if ($link !== false) {
            if ($stmt = mysqli_prepare($link, $sql)) {
                ...
                // Columns in SQL: full_name, father_name, mother_name, email, phone_number, virtual_id, password, whatsapp_number, current_class, previous_class, previous_school, previous_marks_percentage, current_marks, address, pincode, state, photo_filename, roll_number, village, date_of_birth, takes_van (21 total)
                // Types needed: s, s, s, s, s, s, s, s, s, s, s, d, d, s, s, s, s, s, s, s, i
                // Bind Types String: sssssssssssddsssssssi (19 s, 2 d, 1 i = 22 characters. Let's re-verify count)
                // New count: 21 parameters.
                // s (full_name), s (father_name), s (mother_name), s (email), s (phone_number), s (virtual_id), s (password), s (whatsapp_number), s (current_class), s (previous_class), s (previous_school) = 11 's'
                // d (previous_marks), d (current_marks) = 2 'd'
                // s (address), s (pincode), s (state), s (photo_filename), s (roll_number), s (village), s (date_of_birth) = 7 's'
                // i (takes_van) = 1 'i'
                // Total: 11 + 2 + 7 + 1 = 21 parameters.
                // Combined type string: sssssssssssddsssssssi

                $bind_types = "sssssssssssddsssssssi"; // Updated bind types string
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    $email_for_db = null; // NEW: Reset email for DB
    ...
    <div class="bg-white p-8 rounded-xl shadow-2xl w-full max-w-3xl mx-auto">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
            ...
            <div class="md:col-span-2"><h3 class="text-lg font-semibold text-gray-700 border-b pb-2 mb-4 mt-6">Contact Information</h3></div>
             ...
             <!-- NEW: Row for Email Address -->
            <div>
                <label for="email" class="block text-sm font-medium text-gray-700 mb-1">Email Address (Optional)</label>
                <input type="email" name="email" id="email" class="form-control block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm <?php echo (!empty($email_err)) ? 'is-invalid' : ''; ?>" value="<?php echo htmlspecialchars($email); ?>" placeholder="Optional, e.g., student@example.com">
