...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true || ($_SESSION['role'] !== 'admin' && $_SESSION['role'] !== 'principal')) {
    $_SESSION['operation_message'] = "<p class='text-red-600'>Access denied. Only Admins and Principals can edit expenses.</p>";
...
$expense_id = $expense_date = $description = $category = $amount = $payment_method = "";
...
// Validate ID
if ($expense_id === null || !is_numeric($expense_id)) {
     $_SESSION['operation_message'] = "<p class='text-red-600'>Invalid expense ID.</p>";
...
if ($link === false) {
     $operation_message = "<p class='text-red-600'>Database connection error. Could not load expense details.</p>";
     error_log("Edit Expense DB connection failed: " . mysqli_connect_error());
} else {

    // --- Fetch existing expense data ---
    if ($_SERVER["REQUEST_METHOD"] != "POST") { // Only fetch if it's a GET request (not post submission)
         $sql_fetch = "SELECT expense_id, expense_date, description, category, amount, payment_method FROM expenses WHERE expense_id = ?";
         if ($stmt_fetch = mysqli_prepare($link, $sql_fetch)) {
             ...
             if (mysqli_stmt_execute($stmt_fetch)) {
                 ...
                 if (mysqli_num_rows($result_fetch) == 1) {
                     ...
                     $_SESSION['operation_message'] = "<p class='text-red-600'>Expense record not found.</p>";
                     header("location: manage_expenses.php");
                     exit;
                 }
                 mysqli_free_result($result_fetch);
             } else {
                 $operation_message = "<p class='text-red-600'>Error fetching expense data: " . htmlspecialchars(mysqli_stmt_error($stmt_fetch)) . "</p>";
                 error_log("Error executing fetch expense query: " . mysqli_stmt_error($stmt_fetch));
             }
             ...
             $operation_message = "<p class='text-red-600'>Database statement preparation failed for fetching: " . htmlspecialchars(mysqli_error($link)) . "</p>";
              error_log("Error preparing fetch expense statement: " . mysqli_error($link));
         }
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Get hidden expense_id from POST
     $expense_id = $_POST['expense_id'] ?? null;
      if ($expense_id === null || !is_numeric($expense_id)) {
          $operation_message = "<p class='text-red-600'>Invalid request. Expense ID missing on POST.</p>";
      } else {
          $expense_id = (int)$expense_id; // Ensure it's integer
      }
    ...
    if (empty($expense_date_err) && empty($description_err) && empty($amount_err) && empty($payment_method_err) && !empty($expense_id)) {

         // Check if database connection is available
        if ($link === false) {
             $operation_message = "<p class='text-red-600'>Database connection error. Could not update expense.</p>";
             error_log("Edit Expense DB connection failed during update: " . mysqli_connect_error());
        } else {
             // Prepare an update statement
             $sql_update = "UPDATE expenses SET expense_date = ?, description = ?, category = ?, amount = ?, payment_method = ? WHERE expense_id = ?";
             ...
             if ($stmt_update = mysqli_prepare($link, $sql_update)) {
                 ...
                 $param_id_update = $expense_id; // Use the ID from the hidden field

                 // Attempt to execute the prepared statement
                 if (mysqli_stmt_execute($stmt_update)) {
                     // Record updated successfully. Redirect to manage expenses page.
                     $_SESSION['operation_message'] = "<p class='text-green-600'>Expense updated successfully.</p>";
                     header("location: manage_expenses.php");
                     exit;
                 } else {
                     $operation_message = "<p class='text-red-600'>Error updating expense: " . htmlspecialchars(mysqli_stmt_error($stmt_update)) . "</p>";
                      error_log("Error executing update expense query: " . mysqli_stmt_error($stmt_update));
                 }
                 ...
                 $operation_message = "<p class='text-red-600'>Database statement preparation failed for updating: " . htmlspecialchars(mysqli_error($link)) . "</p>";
                  error_log("Error preparing update expense statement: " . mysqli_error($link));
            }
        ...
        // and the errors will be displayed. The $expense_id will also be available from $_POST.
    }
}
    ...
    <!-- Main content wrapper -->
     <div class="w-full max-w-screen-lg mx-auto px-4 sm:px-6 lg:px-8 py-8">
         ...
         <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-8 text-center"><?php echo htmlspecialchars($pageTitle); ?></h1>
          ...
          <!-- Back to Manage Expenses Link -->
         <div class="mb-6 text-center">
             <a href="./manage_expenses.php" class="text-indigo-600 hover:underline text-sm">← Back to Manage Expenses</a>
         </div>
