...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true || $_SESSION['role'] !== 'admin') {
    $_SESSION['operation_message'] = "<p class='text-red-600'>Access denied. Please log in as an admin to view this page.</p>";
    header("location: ../login.php");
    exit;
}

// Get the logged-in user's email from the session (not directly used for auth in admin context now)
// We'll keep it named session_user_email for clarity, though it's not strictly needed for this file's logic.
...
if (!isset($_GET['fee_id']) || !filter_var($_GET['fee_id'], FILTER_VALIDATE_INT)) {
    ...
    if ($link) {
        ...
        if ($stmt_fee = mysqli_prepare($link, $sql_fee)) {
            ...
            if (mysqli_stmt_execute($stmt_fee)) {
                ...
                if (mysqli_num_rows($result_fee) == 1) {
                    ...
                    // --- REMOVED STUDENT-SPECIFIC AUTHORIZATION CHECK FOR ADMIN ---
                    // Since this page is in the 'admin' directory and we've already
                    // confirmed the user is an admin, they are authorized to view any receipt.
                    // The previous check: `if ($fee_details['student_id'] != $session_student_id)`
                    // was incorrectly denying access to admins.
        ...
        // 2. If fee record was found and no general error, fetch student details
        if ($fee_details && empty($error_message)) {
            ...
            if ($stmt_student = mysqli_prepare($link, $sql_student)) {
                // Corrected: Use "s" for string if user_id/student_id are emails/strings.
                // If they are integers, revert to "i". Based on $_SESSION['email'], "s" is more likely.
                mysqli_stmt_bind_param($stmt_student, "s", $student_id_from_fee); 
...
<div class="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6 no-print">
        ...
        <a href="./view_student.php" class="text-sm text-indigo-600 hover:underline font-medium">&larr; Back to Student Profile</a>
    </div>
    ...
    <?php elseif ($receipt_data): ?>
        <div id="receipt-container" class="bg-white rounded-lg shadow-lg overflow-hidden relative border border-gray-200 receipt-container">
            <?php if ($is_paid): ?>
