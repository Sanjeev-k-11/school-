...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true || ($_SESSION['role'] !== 'admin' && $_SESSION['role'] !== 'principal')) {
    $_SESSION['operation_message'] = "<p class='text-red-600'>Access denied. Only Admins  can access this dashboard.</p>";
...
$fetch_monthly_expenses_message = ""; // Message for current month's expenses fetch
$fetch_expense_chart_message = ""; // Message for expense chart data fetch
...
$totalMonthlyExpenses = 0; // Total expenses for the current month

// Variables for Expense Chart Data
$expenseChartLabels = []; // Array for month/year labels (e.g., 'Jan 2023')
$expenseChartData = [];   // Array for total amounts for each month
...
if ($link === false) {
    ...
    $fetch_monthly_expenses_message = "<p class='text-red-600'>Database connection error. Could not load monthly expense total.</p>";
    $fetch_expense_chart_message = "<p class='text-red-600'>Database connection error. Could not load expense chart data.</p>";
    ...
    if (!empty($latest_student_ids)) {
        ...
        // No latest students fetched, total outstanding fee remains 0.
         // $fetch_total_due_latest_5_message = "<p class='text-gray-600 text-sm'>No latest students found to calculate fees for.</p>"; // Can remove if totalOutstandingFeeForLatest5Students defaults to 0
    }
    ...
    // --- Fetch Total Expenses for the Current Month ---
     // Only fetch if admin or principal
    if ($loggedInUserRole === 'admin' || $loggedInUserRole === 'principal') {
         $sql_total_monthly_expenses = "SELECT SUM(amount) AS total_month FROM expenses WHERE YEAR(expense_date) = YEAR(CURDATE()) AND MONTH(expense_date) = MONTH(CURDATE())";

        if ($result_monthly_total = mysqli_query($link, $sql_total_monthly_expenses)) {
            if ($row_monthly_total = mysqli_fetch_assoc($result_monthly_total)) {
                $totalMonthlyExpenses = $row_monthly_total['total_month'] ?? 0; // Use ?? 0 to handle NULL if no expenses
                $fetch_monthly_expenses_message = "Showing expenses for " . date('F Y') . "."; // Informative message
            } else {
                 error_log("Admin Dashboard could not fetch row for total monthly expenses.");
                 $fetch_monthly_expenses_message = "<p class='text-red-600'>Error fetching monthly expense total.</p>";
            }
             mysqli_free_result($result_monthly_total);
        } else {
            $fetch_monthly_expenses_message = "<p class='text-red-600'>Error querying monthly expense total: " . htmlspecialchars(mysqli_error($link)) . "</p>";
            error_log("Admin Dashboard total monthly expenses query failed: " . mysqli_error($link));
        }
    }


     // --- Fetch Monthly Expenses for Chart (Last 12 Months) ---
     // Only fetch if admin or principal
     if ($loggedInUserRole === 'admin' || $loggedInUserRole === 'principal') {
          // Select total amount, year, and month, grouping by year and month
          // Filter for dates starting from 12 months ago from the current date
          // Order by year and month ascending to ensure chronological order
         $sql_expense_chart_data = "SELECT YEAR(expense_date) AS expense_year, MONTH(expense_date) AS expense_month, SUM(amount) AS total_amount
                                    FROM expenses
                                    WHERE expense_date >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)
                                    GROUP BY YEAR(expense_date), MONTH(expense_date)
                                    ORDER BY expense_year ASC, expense_month ASC";

         if ($result_chart = mysqli_query($link, $sql_expense_chart_data)) {
             if (mysqli_num_rows($result_chart) > 0) {
                  // Fetch data and prepare arrays for Chart.js
                 while ($row = mysqli_fetch_assoc($result_chart)) {
                      // Create label like "Jan 2023"
                      $monthName = date('M', mktime(0, 0, 0, $row['expense_month'], 10)); // Get month name
                      $expenseChartLabels[] = $monthName . ' ' . $row['expense_year'];
                      // Add the total amount, ensuring it's numeric (might be string from DB)
                      $expenseChartData[] = (float)$row['total_amount'];
                 }
                 $fetch_expense_chart_message = "Loaded monthly expense data for chart.";
             } else {
                 $fetch_expense_chart_message = "No expense data found for the last 12 months to display in chart.";
             }
             mysqli_free_result($result_chart);
         } else {
              $fetch_expense_chart_message = "<p class='text-red-600'>Error querying expense chart data: " . htmlspecialchars(mysqli_error($link)) . "</p>";
             error_log("Admin Dashboard expense chart query failed: " . mysqli_error($link));
         }
     }
...
?>
        ...
        // --- Background Changer Function (Define here if not global in header) ---
         ...
         document.addEventListener('DOMContentLoaded', function() {
            ...
            updateMasterToggleIcon();
             ...
             if (savedBackgroundClass) {
                 ...
                 if (allowedBackgroundClasses.includes(savedBackgroundClass)) {
                      ...
                      // setBackground('solid-bg-gray'); // Fallback to default - uncomment if you want a default
                 }
             } else {
            ...
            // --- Chart.js Initialization for Expenses ---
             ...
             // Check if the canvas element exists and if Chart.js is loaded
             if (expenseChartCanvas && typeof Chart !== 'undefined') {
                  // Get chart data from PHP variables embedded in the script
                  // Use JSON.parse to convert the JSON strings into JavaScript arrays
                       ...
                       new Chart(ctx, {
                           ...
                           options: {
                               ...
                               plugins: {
                                   ...
                                   title: {
                                       display: true,
                                       text: 'Monthly Expenses (Last 12 Months)',
                                        font: { size: 16 }
                                   }
                               }
                           }
                       });
                       console.log("Expense chart initialized."); // Debugging
                  } else {
                       // Display message if no data for chart
                       const chartContainer = expenseChartCanvas.parentElement;
                       if(chartContainer) {
                            chartContainer.innerHTML = `<div class="text-center text-gray-600 p-4">No sufficient expense data available for the chart.</div>`;
                            console.log("No data to draw expense chart.");
                       } else {
                            console.warn("Chart canvas parent not found.");
                       }
                  }
             } else {
                  console.warn("Chart canvas element #expenseChart not found or Chart.js is not loaded.");
             }
    ...
    <!-- Main content wrapper -->
     ...
     <div class="w-full max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8"> <!-- Responsive padding: w-full makes it take full width, max-w-screen-2xl limits max width on huge screens, mx-auto centers it. -->
         ...
         <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
             ...
             <?php if ($loggedInUserRole === 'admin' || $loggedInUserRole === 'principal'): // Only show financial info to admin/principal ?>
                 ...
                 <!-- Added Red border for expenses -->
                 <div class="flex items-center p-6 rounded-xl shadow-md border-b-4 border-red-500 bg-white">
                      <!-- Icon placeholder - Shopping Cart or Receipt -->
                     <div class="flex-shrink-0 p-3 bg-red-100 rounded-full mr-4">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                               <path stroke-linecap="round" stroke-linejoin="round" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                           </svg>
                     </div>
                     <div>
                          <h3 class="text-base font-medium text-gray-600">Total Expenses This Month</h3>
                           <!-- Format as currency -->
                          <p class="text-2xl font-bold text-gray-800">₹ <?php echo number_format($totalMonthlyExpenses, 2); ?></p>
                           <?php // Display fetch message/error if applicable ?>
                           <?php if (!empty($fetch_monthly_expenses_message)): ?>
                                <p class="text-xs text-gray-500 mt-1"><?php echo htmlspecialchars($fetch_monthly_expenses_message); ?></p>
                           <?php endif; ?>
                           <!-- Add link to manage expenses -->
                           <p class="text-xs text-gray-500 mt-1">
                               <a href="./manage_expenses.php" class="text-indigo-600 hover:underline">Manage Expenses</a>
                           </p>
                     </div>
                 </div>
         ...
         <div class="flex flex-wrap lg:flex-nowrap gap-8"> <!-- This flex container controls the side-by-side layout on large screens -->

            <!-- --- Left/Main Content Column (Student/Staff Tables & Charts) --- -->
            ...
            <div id="leftColumn" class="flex-1 w-full lg:w-2/3 min-w-[300px] flex flex-col gap-8"> <!-- flex-1 and w-full/lg:w-2/3 make this column responsive -->

                 <?php if ($loggedInUserRole === 'admin' || $loggedInUserRole === 'principal'): // Only show charts to admin/principal ?>
                      <!-- Expense Chart Section -->
                      <div class="bg-white p-6 rounded-xl shadow-md">
                          <h2 class="text-xl sm:text-2xl font-semibold text-gray-800 mb-6 text-center">Monthly Expenses Overview</h2>

                           <!-- Expense Chart Data Message -->
                          <?php
                           if (!empty($fetch_expense_chart_message)) {
                               // Determine message type
                               $message_classes = "p-3 rounded-md border mb-4 text-center text-sm ";
                               $msg_lower = strtolower(strip_tags($fetch_expense_chart_message));
                               ...
                               } elseif (strpos($msg_lower, 'no expense data found') !== false) {
                                    $message_classes .= "bg-yellow-100 border-yellow-300 text-yellow-800"; // Warning for no data
                               } else { // Default to info
                                   $message_classes .= "bg-blue-100 border-blue-300 text-blue-800"; // Info
                               }
                                echo "<div class='{$message_classes}' role='alert'>" . htmlspecialchars($fetch_expense_chart_message) . "</div>";
                           }
                          ?>

                          <!-- Chart Container -->
                           <!-- Add a specific height or aspect ratio via CSS if needed, or let Chart.js handle it with responsive: true -->
                          <div class="chart-container"> <!-- The CSS for chart-container sets width: 100% -->
                              <canvas id="expenseChart"></canvas> <!-- Chart.js renders here -->
                          </div>
                      </div>
                 <?php endif; ?>
                 ...
                 <div class="bg-white p-6 rounded-xl shadow-md">
                      ...
                      <?php if (!empty($students)): ?>
                          ...
                          <!-- Student Table -->
                          <div class="overflow-x-auto"> <!-- overflow-x-auto enables horizontal scrolling if table content is too wide -->
                              <table class="min-w-full divide-y divide-gray-200 data-table" id="studentTable"> <!-- min-w-full helps ensure table doesn't shrink below content -->
                                  <thead>
                                      <tr>
                 ...
                 <div class="bg-white p-6 rounded-xl shadow-md">
                      ...
                      <?php if (!empty($staff_members)): ?>
                          ...
                          <!-- Staff Table -->
                          <div class="overflow-x-auto"> <!-- overflow-x-auto for horizontal scrolling -->
                              <table class="min-w-full divide-y divide-gray-200 data-table" id="staffTable"> <!-- min-w-full helps table fill space -->
                                  <thead>
                                      <tr>
                                          <th scope="col" class="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider bg-blue-50 rounded-tl-lg">Photo</th> <!-- Rounded corner -->
            ...
            <div id="rightColumn" class="w-full lg:w-1/3 min-w-[250px] max-w-[400px] flex flex-col gap-8"> <!-- w-full/lg:w-1/3, min/max-width make this column responsive -->
                 ...
                 <div id="otherInfoSection" class="bg-white p-6 rounded-xl shadow-md flex flex-col">
                      <!-- Header with title and toggle button -->
                       <!-- Added class 'toggle-button-area' for modal click prevention -->
                     <div class="flex justify-between items-center mb-4 toggle-button-area">
                        <h2 class="text-xl font-semibold text-gray-800">Quick Links & Info</h2>
                         <!-- Toggle button -->
                          <!-- Added type="button" -->
                     ...
                     <div id="otherInfoContent">
                         <p class="text-gray-700 text-sm leading-normal break-words mb-4">
                             This area provides quick access to common tasks and information.
                         </p>
                         <ul class="list-disc list-inside text-sm text-blue-600">
                             ...
                             <?php if ($loggedInUserRole === 'admin' || $loggedInUserRole === 'principal'): // Add expense links ?>
                                 <li class="mt-2"><a href="./add_expense.php" class="hover:underline text-green-600 font-semibold">Add New Expense</a></li>
                                 <li><a href="./manage_expenses.php" class="hover:underline text-indigo-600 font-semibold">Manage Expenses</a></li>
                             <?php endif; ?>
                             <!-- Add other relevant quick links here -->
                         </ul>
